...
@Component({
  selector: 'app-profit-loss-market-user',
  templateUrl: './profit-loss-market-user.component.html',
  styleUrls: ['./profit-loss-market-user.component.scss']
})
export class ProfitLossMarketUserComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.column = [
      ...
      },
      {
        ...
        "render": function (data, type, row) {
          var content = ''
          if (row.plUser != null && row.plUser < 0) {
            content = content + '<span class="text-center text-danger">' + ((row.plUser).toFixed(2)) + '</span>';
          } else if (row.plUser != null && row.plUser >= 0) {
            content = content + '<span class="text-center text-success">' + ((row.plUser).toFixed(2)) + '</span>';
          } else {
            content = '';
          }
          return content;
      ...
      { 
        ...
        "render": function (data, type, row) {
          var content = ''
          if (row.comUser != null && row.comUser < 0) {
            content = content + '<span class="text-center text-danger">' + (row.comUser).toFixed(2) + '</span>';
          } else if (row.comUser != null && row.comUser >= 0) {
            content = content + '<span class="text-center text-success">' + (row.comUser).toFixed(2) + '</span>';
          } else {
            content = '';
          }
          return content;
        ...
        .subscribe(resp => {
            if(resp["status"]){
              ...
              for(let i = 0 ; i< this.profitData.length; i++){
                ...
                this.profitData[i].plUser = -(this.profitData[i].p_l_full)
                this.profitData[i].comUser = -(this.profitData[i].commission_full)
                if(this.sportId < 0){
  ...
  userBet(data){
      ...
      if(data.sport_id > 0){
        // let b = 'http://192.168.0.8:4100/'+ 'plmarketUserBet/' + data.match_id + '/'+data.event_id + '/'+data.match_name + '/'+data.sport_name + '/'+ this.user_id  + '/'+ data.type;
        let b = a[0] + 'plmarketUserBet/' + data.match_id + '/'+data.event_id + '/'+data.match_name + '/'+data.sport_name + '/'+ this.user_id  + '/'+ data.type;
        window.open(b, '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
      } else {
        if (data.sport_id == '-102') {
          // let b = 'http://192.168.0.8:4100/' + 'profitLossSnUserBet/' + data.match_id + '/' + data.event_id + '/' + data.match_name + '/' + data.sport_name + '/' + this.user_id + '/' + data.type + '/'+ data.event_name;
          let b = a[0] + 'profitLossSnUserBet/' + data.match_id + '/' + data.event_id + '/' + data.match_name + '/' + data.sport_name + '/' + this.user_id + '/' + data.type + '/'+ data.event_name;
          window.open(b, '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
        }
      }
